#!/usr/bin/env python3
"""Local auth-injecting proxy for NVIDIA OpenAI-compatible endpoints.

Why: current Copilot customoai path may omit Authorization header for some setups.
This proxy accepts local requests without auth and forwards them to NVIDIA with
...
class ProxyHandler(BaseHTTPRequestHandler):
    server_version = "NVIDIAAuthProxy/1.0"
    ...
    @staticmethod
    def _is_minimax_tool_stream_request(request_json: dict | None) -> bool:
        if not request_json:
            return False

        model = request_json.get("model")
        if not isinstance(model, str) or not model.startswith("minimaxai/"):
            return False

        has_tools = isinstance(request_json.get("tools"), list) and len(request_json.get("tools") or []) > 0
        wants_stream = bool(request_json.get("stream"))
        return has_tools and wants_stream
    ...
    def _forward(self) -> None:
        ...
        force_minimax_tool_non_stream = self._is_minimax_tool_stream_request(req_json)
        if force_minimax_tool_non_stream and req_json is not None:
        ...
        try:
            with urllib.request.urlopen(req, timeout=120) as res:
                ...
                # MiniMax streamed tool calls can emit argument chunks that break Copilot JSON parsing.
                # We request non-stream upstream and re-emit a stable synthetic SSE response downstream.
                if force_minimax_tool_non_stream:
                    payload = res.read()
                    if sanitize and "application/json" in (content_type or "").lower():
                        payload = self._sanitize_json_payload(payload)

                    completion = self._load_json_payload(payload)
                    if not completion:
                        raise ValueError("Failed to parse upstream completion payload for synthetic SSE mode")
                    ...
                    self.send_header("Connection", "close")
                    self.end_headers()
                    self._emit_synthetic_sse_from_completion(completion)
                    response_bytes = max(response_bytes, len(payload))
                    return
    ...
    def log_message(self, fmt: str, *args) -> None:
        ...
        print(f"[proxy {ts}] {self.address_string()} - {fmt % args}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NVIDIA auth proxy")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8787)
    parser.add_argument("--target-base", default="https://integrate.api.nvidia.com")
    ...
    if not args.api_key:
        raise SystemExit("Missing API key. Pass --api-key or set NVIDIA_API_KEY.")
    ...
    print(f"NVIDIA auth proxy listening on http://{args.host}:{args.port}")

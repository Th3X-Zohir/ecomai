...
export default function DriverAssignments() {
  ...
  const handleAssign = async (e) => {
    e.preventDefault();
    if (!driverId.trim()) { setError('Please enter a driver user ID'); return; }
  ...
  const columns = [
    ...
    { key: 'driver', label: 'Driver', render: (r) => r.driver_user_id ? (
      <span className="text-sm text-gray-700 font-mono">{r.driver_user_id.slice(0, 10)}...</span>
    ) : (
    ...
    { key: 'actions', label: '', render: (r) => (
      (r.status === 'requested' || r.status === 'assigned') ? (
        <Button size="xs" variant="secondary" onClick={(e) => { e.stopPropagation(); setAssigning(r); setDriverId(r.driver_user_id || ''); setError(''); }}>
      ...
      <Modal open={!!assigning} onClose={() => setAssigning(null)} title="Assign Driver" size="sm">
        ...
        {assigning && (
          <form onSubmit={handleAssign}>
            ...
            <FormField label="Driver User ID" required>
              <input
                type="text"
                value={driverId}
                onChange={(e) => setDriverId(e.target.value)}
                placeholder="Enter driver user UUID"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
              />
              <p className="text-xs text-gray-400 mt-1">The UUID of a user with the delivery_agent role</p>
            </FormField>

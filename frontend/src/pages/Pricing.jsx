...
const faqs = [
  { q: 'Can I start for free?', a: 'Absolutely. Our Free plan gives you everything you need to launch your online store — no credit card required. Upgrade anytime as your business grows.' },
  { q: 'Can I switch plans at any time?', a: 'Yes. Upgrade or downgrade your plan at any time from your dashboard. Changes take effect immediately.' },
  { q: 'What payment methods do you support?', a: 'All plans support bKash, Nagad, SSLCommerz, and cash on delivery. These are integrated directly — no extra setup needed.' },
  { q: 'Is there a long-term contract?', a: 'No contracts. All plans are month-to-month. Choose yearly billing to save 17% — cancel anytime.' },
  { q: 'What happens if I exceed my plan limits?', a: "We'll notify you as you approach limits. Your store keeps running — we never interrupt a sale mid-transaction." },
  { q: 'Can I import existing products?', a: 'Yes. All plans support CSV/Excel bulk import. Enterprise plans include assisted data migration from other platforms.' },
  { q: 'Do you offer refunds?', a: 'We offer a 14-day refund on paid plans if you\'re not satisfied. Email our support team and we\'ll process it within 24 hours.' },
  { q: 'What support is available?', a: 'Free and Starter plans include email support. Growth plans get priority response. Enterprise includes a dedicated account manager.' },
];

const trustSignals = [
  {
    icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z',
    label: 'SSL-encrypted checkout',
  },
  {
...
export default function Pricing() {
      ...
      .then(data => {
        ...
        if (items.length) {
          const processed = items.map(p => ({
            ...
            icon: PLAN_ICONS[p.slug] || PLAN_ICONS.enterprise,
          }));
          setPlans(processed);
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      {/* ═══════════════════ NAVBAR ═══════════════════ */}
      <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 backdrop-blur-2xl shadow-lg shadow-gray-200/40 border-b border-gray-200/50' : 'bg-white/90 backdrop-blur-md border-b border-gray-100'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2.5 group">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-teal-500 flex items-center justify-center shadow-lg shadow-primary-200/50">
      ...
      <section ref={heroRef} className="pt-28 pb-6 sm:pt-36 sm:pb-10 text-center relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <div className="absolute -top-[20%] left-1/2 -translate-x-1/2 w-[700px] h-[500px] bg-primary-100/30 rounded-full blur-[100px]" />
          <div className="absolute bottom-0 right-0 w-[300px] h-[300px] bg-teal-100/20 rounded-full blur-[80px]" />
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6">
          <div style={reveal(heroVis)} className="inline-flex items-center gap-2.5 bg-primary-50 border border-primary-100 rounded-full px-5 py-2 mb-8 shadow-sm">
            <svg className="w-4 h-4 text-primary-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            <span className="text-xs font-semibold text-primary-700">Start free — no credit card required</span>
          </div>
          ...
          <h1 style={reveal(heroVis, 0.1)} className="text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-gray-900 leading-[1.1]">
            Simple, transparent
            <br />
            <span className="bg-gradient-to-r from-primary-600 to-teal-500 bg-clip-text text-transparent">Pricing</span>
          </h1>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {plans.map((plan, idx) => {
              ...
              return (
                ...
                >
                  {isPopular && (
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-primary-600 to-teal-500 text-white text-xs font-bold px-6 py-2 rounded-full shadow-lg shadow-primary-200/50">
                      MOST POPULAR
                    </div>
                  )}
                  ...
                  <div className="mb-7">
                    {isEnterprise ? (
                      <>
                        ...
                        <span className="block text-sm text-gray-500 mt-1">Talk to our sales team</span>
                      </>
                    ) : (
                      <>
                        ...
                        {price === 0 && (
                          <span className="block text-xs text-emerald-500 font-semibold mt-1.5">Free forever</span>
                        )}
                      </>
                    )}
                  </div>
                  ...
                  {/* CTA button */}
                  <Link
                    to={isEnterprise ? 'mailto:sales@ecomai.dev' : `/signup?plan=${plan.slug}${yearly ? '&billing=yearly' : ''}`}
                    className={`relative w-full text-center py-3.5 rounded-xl font-semibold transition-all duration-300 overflow-hidden group/btn flex items-center justify-center gap-2 ${
                      isPopular
      ...
      <section ref={tableRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        <div className="text-center mb-14" style={reveal(tableVis)}>
          ...
          <p className="text-gray-500 mt-3 max-w-lg mx-auto">Every plan includes SSL, hosting, and basic support. Upgrade anytime.</p>
        </div>
              ...
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                {plans.map(p => (
                  <th key={p.slug} className={`text-center text-sm font-bold py-5 px-4 ${p.is_popular ? 'text-primary-600 bg-primary-50/50' : 'text-gray-900'}`}>
                    {p.name}
                    {p.is_popular && <span className="ml-2 text-[10px] bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full font-semibold">Popular</span>}
                  </th>
                ))}
              </tr>
            </thead>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14" style={reveal(faqVis)}>
            ...
            <p className="text-gray-500 mt-3">
              ...
              <a href="mailto:support@ecomai.dev" className="text-primary-600 font-semibold hover:underline">Contact our team</a>.
            </p>
          </div>
      ...
      <section ref={ctaRef} className="py-16 sm:py-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative rounded-[2rem] overflow-hidden p-12 sm:p-16 text-center bg-gradient-to-br from-primary-600 via-primary-600 to-teal-600" style={reveal(ctaVis)}>
            <div className="absolute top-0 right-0 w-72 h-72 rounded-full opacity-10 blur-3xl bg-white" />
            <div className="absolute bottom-0 left-0 w-56 h-56 rounded-full opacity-10 blur-3xl bg-white" />
            ...
            <p className="relative text-lg text-white/80 max-w-xl mx-auto mb-10">
              Set up your online store in minutes. No credit card, no contracts, no commitment.
            </p>
            <div className="relative flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <a href="mailto:sales@ecomai.dev" className="w-full sm:w-auto border-2 border-white/30 text-white px-8 py-4 rounded-2xl text-base font-semibold hover:bg-white/10 hover:border-white/50 transition-all flex items-center justify-center">
                Talk to Sales
              </a>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-8 pb-14 border-b border-gray-800/80">
            <div className="col-span-2">
              ...
              <p className="text-sm leading-relaxed mb-6 max-w-xs">The e-commerce platform built for Bangladesh. Accept payments, manage deliveries, and grow your business.</p>
            ...
            {[
              { title: 'Product', links: [['Features', '/'], ['Pricing', '/pricing']] },
              { title: 'Company', links: [['About', '#'], ['Blog', '#'], ['Contact', '#']] },
              { title: 'Resources', links: [['Help Center', '#'], ['API Reference', '#']] },
